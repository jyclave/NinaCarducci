...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(lightboxId){const activeImage=$(".lightboxImage").attr("src");const activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){imagesCollection=$("img.gallery-item").toArray()}else{imagesCollection=$("img.gallery-item").filter(function(){return $(this).data("gallery-tag")===activeTag}).toArray()}
const currentIndex=imagesCollection.findIndex(img=>$(img).attr("src")===activeImage);const prevIndex=currentIndex===0?imagesCollection.length-1:currentIndex-1;$(".lightboxImage").attr("src",$(imagesCollection[prevIndex]).attr("src"))},nextImage(lightboxId){const activeImage=$(".lightboxImage").attr("src");const activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){imagesCollection=$("img.gallery-item").toArray()}else{imagesCollection=$("img.gallery-item").filter(function(){return $(this).data("gallery-tag")===activeTag}).toArray()}
const currentIndex=imagesCollection.findIndex(img=>$(img).attr("src")===activeImage);const nextIndex=currentIndex===imagesCollection.length-1?0:currentIndex+1;$(".lightboxImage").attr("src",$(imagesCollection[nextIndex]).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
...
$(".tags-bar .nav-link").removeClass("active");$(".active-tag").removeClass("active-tag");$(this).addClass("active active-tag");var tag=$(this).data("images-toggle");$(".gallery-item").each(function(){$(this).parents(".item-column").hide();if(tag==="all"){$(this).parents(".item-column").show(300)}else if($(this).data("gallery-tag")===tag){$(this).parents(".item-column").show(300)}})}}})(jQuery)